defmodule SymphonyElixir.Orchestrator do
  ...
  alias SymphonyElixir.{AgentRunner, Config, StatusDashboard, TokenUsageLog, Tracker, Workspace}
    ...
    case find_issue_id_for_ref(running, ref) do
      ...
      issue_id ->
        ...
        state =
          case reason do
            :normal ->
              continuation_attempt = next_retry_attempt_from_running(running_entry)
  ...
  def handle_info(
    ...
    case Map.get(running, issue_id) do
      ...
      running_entry ->
        ...
        TokenUsageLog.record(update, running_entry, updated_running_entry, token_delta, state.codex_totals)
  ...
  defp handle_active_retry(state, issue, attempt, %{delay_type: :continuation})
       when is_integer(attempt) and attempt > @max_continuation_retries do
    Logger.warning("Max continuation retries reached for #{issue_context(issue)} attempt=#{attempt} max=#{@max_continuation_retries}; releasing claim")
  ...
  defp retry_delay(attempt, %{delay_type: :continuation}) when is_integer(attempt) and attempt > 0 do
